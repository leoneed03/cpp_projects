...
namespace bplus_tree_namespace{
    ...
    Bptree<T1,T2>::Bptree(int n){
        this->t = n;
        this->root = nullptr;
    }
    
    template <class T1, class T2>
    Bptree<T1,T2>::Bptree(int n, node<T1,T2>& Node) {
        this->t = n;
        this->root = Node;
    }
    ...
    template <class T1, class T2>
    node<T1,T2>* Bptree<T1,T2>::find_leaf(T1 key) {
    ...
    template <class T1, class T2>
    bool Bptree<T1,T2>::insert(const T1& key, const T2& value) {
        if (!this->root)
    ...
    template <class T1, class T2>
    void Bptree<T1,T2>::split (node<T1,T2>* Node) {
...
template <class T1, class T2>
    bool Bptree<T1,T2>::delete_check(const T1& key) {
        node<T1,T2>* leaf = find_leaf(key);
        int pos = 0;
        bool f = false;
        for (int i = 0; i < leaf->get_key_num(); i++) {
...
template <class T1, class T2>
    int Bptree<T1,T2>::child_num(node<T1,T2>& Node){
        int res = 0;
...
template <class T1, class T2>
    void Bptree<T1,T2>::delete_key(node<T1,T2>& Node, const T1& key, int p) {
        if (!(&Node))
                return;
        bool f = false;
        for (int i = 0; i < 2 * t; i++) {
...
template <class T1, class T2>
    T1 Bptree<T1,T2>::min_key(node<T1,T2>* Node) {
...
template <class T1, class T2>
    void Bptree<T1,T2>::update(node<T1,T2>* Node) {
        T1 Key = min_key(Node);
        while (Node){
            int i = 0;
...
template <class T1, class T2>
    void Bptree<T1,T2>::lazy_insert_left(node<T1,T2>* Node, node<T1,T2>* left, int pos, const T1& temp, const T2& val){
...
template <class T1, class T2>
    void Bptree<T1,T2>::lazy_insert_right(node<T1,T2>* Node, node<T1,T2>* right, int pos, const T1& temp, const T2& val ){
        int key_num = Node->get_key_num() - 1;
        if (Node->get_key_num() < 2 * t - 1){
...
template <class T1, class T2>
    bool Bptree<T1,T2>::check_for_deleted(node<T1,T2>* for_check) {
...
template <class T1, class T2>
    void Bptree<T1,T2>::insert_in_deleted(node<T1,T2>* for_insert, const T1& key, const T2& value) {
        int pos = 0;
...
template <class T1, class T2>
    node<T1,T2>* Bptree<T1,T2>::fix_node(node<T1,T2>* for_fix) {
        int t = this->t;
...
template <class T1, class T2>
    void Bptree<T1,T2>::lazy_insert(const T1& key, const T2& val){
...
template <class T1, class T2>
    int Bptree<T1,T2>::find_pos(node<T1,T2>* find_in, const T1& key){
        int pos = 0;
...
template <class T1, class T2>
    int Bptree<T1,T2>::lazy_delete(const T1& key){
        node<T1,T2>* leaf = find_leaf(key);
...
template <class T1, class T2>
    void Bptree<T1,T2>::pr(){
        p(this->root, 0);
    }
    
template <class T1, class T2>
    void Bptree<T1,T2>::p(node<T1,T2>* cur, int level){
        int i = 0;
...
template<class T1, class T2>
    std::vector<T1> Bptree<T1,T2>::print() {
        std::vector<T1> res;
        node<T1,T2>* cur = this->root;
        for (int i = 0; i < cur->get_key_num(); i++){
